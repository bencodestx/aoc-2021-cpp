...
#include "aoc2021/day3/part1.hpp"
#include "aoc2021/day3/part2.hpp"
...
static constexpr auto sample = R"(00100
11110
10110
10111
10101
...
TEST(Day3, Part1) {
  ...
  EXPECT_EQ(198, aoc2021::day3::part1<5>(in));
}
...
// next bit to the right. The bit criteria depends on which type of rating value
// you want to find:
//
...
// being considered. To find CO2 scrubber rating, determine the least common
// value (0 or 1) in the current bit position, and keep only numbers with that
// bit in that position. If 0 and 1 are equally common, keep values with a 0 in
// the position being considered. For example, to determine the oxygen generator
// rating value using the same example diagnostic report from above:
//
...
// fourth position, two of the three numbers have a 1, so keep those two: 10110
// and 10111. In the fifth position, there are an equal number of 0 bits and 1
// bits (one each). So, to find the oxygen generator rating, keep the number
// with a 1 in that position: 10111. As there is only one number left, stop; the
// oxygen generator rating is 10111, or 23 in decimal. Then, to determine the
...
// second position: 01111 and 01010. In the third position, there are an equal
// number of 0 bits and 1 bits (one each). So, to find the CO2 scrubber rating,
// keep the number with a 0 in that position: 01010. As there is only one number
// left, stop; the CO2 scrubber rating is 01010, or 10 in decimal. Finally, to
// find the life support rating, multiply the oxygen generator rating (23) by
...
TEST(Day3, Part2) {
  ...
  EXPECT_EQ(230, aoc2021::day3::part2<5>(in));
}
